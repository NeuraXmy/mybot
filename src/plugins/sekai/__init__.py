...
# 统一获取解包图片资源
async def get_asset(path: str, cache=True, allow_error=False, default=UNKNOWN_IMG, cache_expire_secs=None, timeout=5) -> Image.Image:
...
async def get_not_image_asset(path: str, cache=True, allow_error=False, default=None, binary=False, cache_expire_secs=None):
    ...
    def try_load_from_cache(use_exp):
        try:
            if not cache: raise
            if use_exp and os.path.exists(cache_path) and cache_expire_secs \
                and datetime.now().timestamp() - os.path.getmtime(cache_path) > cache_expire_secs:
                    raise
    ...
    if data := try_load_from_cache(True):
        return data
...
async def get_card_image(cid, after_training):
    ...
    card = find_by(await res.cards.get(), "id", cid)
    if not card: raise Exception(f"找不到ID为{cid}的卡牌") 
    return await get_asset(f"character/member/{card['assetbundleName']}_rip/card_{image_type}.png", timeout=30)
...
async def get_mysekai_fixture_genre_name_and_image(gid, is_main_genre):
    ...
    image = await get_asset(f"mysekai/icon/category_icon/{asset_name}_rip/{asset_name}.png", allow_error=True)
...
async def load_my_sekai_run_data():
    global mysekairun_data, mysekairun_mtime
    path = "data/sekai/Mysekai IKEA.html"
